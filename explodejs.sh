...
if [ -f "$CONFIGPATH" ] && [ -f "$FILEPATH" ]; then
    ...
    if [ $SILENT_OP = true ]; then
        ...
        npm run start --prefix parser --cache-directory "$NPM_CACHE_DIR" -- -f "$FILEPATH" -c "$CONFIGPATH" -o "$NORMALIZED" -g "$GRAPH_DIR" --csv --cache-directory "$NPM_CACHE_DIR"
    else
        ...
        npm run start --prefix parser --cache-directory "$NPM_CACHE_DIR" -- -f "$FILEPATH" -c "$CONFIGPATH" -o "$NORMALIZED" -g "$GRAPH_DIR" --csv --cache-directory "$NPM_CACHE_DIR" | tee "$NORM"
    fi
