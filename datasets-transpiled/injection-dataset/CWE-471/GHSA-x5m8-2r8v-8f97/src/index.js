...
function set(obj, path, value) {
  ...
  for (var i = 0; i < path.length; i++) if (i === path.length - 1) obj[path[i]] = value;else if (null == obj[path[i]]) obj = obj[path[i]] = isNonNegativeInteger(path[i + 1]) ? [] : {};else if (!isPrototypePolluted(path[i])) obj = obj[path[i]];
  return value;
}
  ...
  if (Array.isArray(obj)) {
    ...
    for (var k = 0; k < obj.length; k++) {
      ...
      if (isObject(v, includeArrays)) {
        if (false === each(v, iter, includeArrays, path.concat(k))) return false;
      } else {
