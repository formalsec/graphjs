...
class tools extends emitter {
  ...
  funcs(opts) {
    ...
    if (opts.range.length <= 0) return "Range of hosts could not be created";
    ...
    Object.keys(opts.range).forEach(function blocks(block) {
      ...
      funcs[range] = function block(callback) {
        ...
        if (opts.verbose) console.log(`Running: ${cmd}`);

        const report = [];

        const execute = proc(cmd, (err, stdout, stderr) => {
          if (err) return reporting.reports(opts, err, callback);
        });
        ...
        execute.stdout.on('end', () => {
          if (report.length > 0) return reporting.reports(opts, report, callback);
        });
      };
    });
  ...
  command(opts, block) {
    ...
    const ipv6 = validation.test(validation.patterns.IPv6, block) ? ' -6 ' : ' ';

    return opts.ports ? `${opts.nmap + proto} ${to}${flags}${ipv6}-p${opts.ports} ${block}` : `${opts.nmap + proto} ${to}${flags}${ipv6}${block}`;
  }
  ...
  init(defaults, opts, cb) {
    ...
    if (/array/.test(typeof opts.flags)) defaults.flags = opts.flags;

    opts = this.merge(defaults, opts);

    /* Ensure we can always parse the report */
    if (opts.flags.indexOf('-oX -') === -1) opts.flags.push('-oX -');
    ...
    if (/nmap.discover/.test(called)) {
      ...
      if (!(opts.range = network.adapters(opts))) return cb(validation.messages.version);

      /* Set scan options as static values for 'discover' mode */
      opts.ports = '';
      opts.flags = ['-n', '-oX -', '-sn', '-PR'];
    }
