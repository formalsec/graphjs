...
var gitQueue = async.queue(function (task, callback) {
  ...
  var process = child_process.exec(task.command, {
    cwd: task.repoPath,
    ...
    if (error !== null) {
      ...
      err.stderr = stderr;
      err.stdout = stdout;
      if (stderr.indexOf('Not a git repository') >= 0) err.errorCode = 'not-a-repository';else if (err.stderr.indexOf('Connection timed out') != -1) err.errorCode = 'remote-timeout';else if (err.stderr.indexOf('Permission denied (publickey)') != -1) err.errorCode = 'permision-denied-publickey';else if (err.stderr.indexOf('ssh: connect to host') != -1 && err.stderr.indexOf('Bad file number') != -1) err.errorCode = 'ssh-bad-file-number';else if (err.stderr.indexOf('No remote configured to list refs from.') != -1) err.errorCode = 'no-remote-configured';else if (err.stderr.indexOf('unable to access') != -1 && err.stderr.indexOf('Could not resolve host:') != -1 || err.stderr.indexOf('Could not resolve hostname') != -1) err.errorCode = 'offline';else if (err.stderr.indexOf('Proxy Authentication Required') != -1) err.errorCode = 'proxy-authentication-required';else if (err.stderr.indexOf('Please tell me who you are') != -1) err.errorCode = 'no-git-name-email-configured';else if (err.stderr.indexOf('FATAL ERROR: Disconnected: No supported authentication methods available (server sent: publickey)') == 0) err.errorCode = 'no-supported-authentication-provided';else if (stderr.indexOf('fatal: No remote repository specified.') == 0) err.errorCode = 'no-remote-specified';else if (err.stderr.indexOf('non-fast-forward') != -1) err.errorCode = 'non-fast-forward';else if (err.stderr.indexOf('Failed to merge in the changes.') == 0 || err.stdout.indexOf('CONFLICT (content): Merge conflict in') != -1 || err.stderr.indexOf('after resolving the conflicts') != -1) err.errorCode = 'merge-failed';else if (err.stderr.indexOf('This operation must be run in a work tree') != -1) err.errorCode = 'must-be-in-working-tree';else if (err.stderr.indexOf('Your local changes to the following files would be overwritten by checkout') != -1) err.errorCode = 'local-changes-would-be-overwritten';
      task.setResult(err);
      callback(err);
    } else {
...
git.status = function (repoPath, file) {
  ...
  var gitTask = git('status -s -b -u ' + (file ? '"' + file + '"' : ''), repoPath).parser(gitParser.parseGitStatus).fail(task.setResult).done(function (status) {
    // From http://stackoverflow.com/questions/3921409/how-to-know-if-there-is-a-git-rebase-in-progress
    status.inRebase = fs.existsSync(path.join(repoPath, '.git', 'rebase-merge')) || fs.existsSync(path.join(repoPath, '.git', 'rebase-apply'));
...
git.getRemoteAddress = function (repoPath, remoteName) {
  return git('config --get remote.' + remoteName + '.url', repoPath).parser(function (text) {
...
git.stashAndPop = function (repoPath, wrappedTask) {
  ...
  var gitTask = git('stash', repoPath).always(function (err, res) {
    ...
    if (err) {
      ...
      if (res.indexOf('No local changes to save') != -1) hadLocalChanges = false;
    }
    if (hadLocalChanges) {
      var popTask = git('stash pop', repoPath).always(task.setResult);
      wrappedTask.always(function () {
        popTask.start();
      });
    } else {
...
git.binaryFileContent = function (repoPath, filename, version) {
  return git('show ' + version + ':' + filename, repoPath).encoding('binary');
};

git.diffFile = function (repoPath, filename, sha1, maxNLines) {
  var task = new GitTask();

  var statusTask = git.status(repoPath).fail(task.setResult).done(function (status) {
    var file = status.files[filename];
    var filePath = path.join(repoPath, filename);
    if (!file && !sha1) {
      if (fs.existsSync(path.join(repoPath, filename))) task.setResult(null, []);else task.setResult({ error: 'No such file: ' + filename, errorCode: 'no-such-file' });
      ...
      git(gitCommand, repoPath).parser(gitParser.parseGitDiff, { maxNLines: maxNLines }).always(task.setResult).start();
    } else {
      fs.readFile(filePath, { encoding: 'utf8' }, function (err, text) {
        ...
        diff.lines = lines.map(function (line, i) {
          return [null, i, '+' + line];
        });
        diffs.push(diff);
...
git.discardAllChanges = function (repoPath) {
  ...
  var gitTask = git('reset --hard HEAD', repoPath).fail(task.setResult).done(function () {
...
git.discardChangesInFile = function (repoPath, filename) {
  ...
  var statusTask = git.status(repoPath, filename).fail(task.setResult).done(function (status) {
    ...
    if (!fileStatus.staged) {
      ...
      if (fileStatus.isNew) {
        fs.unlink(filePath, function (err) {
          if (err) task.setResult({ command: 'unlink', error: err });else task.setResult();
        });
        // If it's a changed file, reset the changes
      } else {
        git('checkout HEAD -- "' + filename.trim() + '"', repoPath).always(task.setResult).start();
      }
    } else {
...
git.updateIndexFromFileList = function (repoPath, files) {
  ...
  var statusTask = git.status(repoPath).fail(task.setResult).done(function (status) {
    ...
    for (var v in files) {
      ...
      if (fileStatus.removed) toRemove.push(file);else toAdd.push(file);
    }

    async.series([function (done) {
      if (toAdd.length == 0) done();else {
        git('update-index --add --stdin', repoPath).always(done).started(function () {
          var filesToAdd = toAdd.map(function (file) {
            return file.trim();
          }).join('\n');
      ...
      if (toRemove.length == 0) done();else {
        git('update-index --remove --stdin', repoPath).always(done).started(function () {
          var filesToRemove = toRemove.map(function (file) {
            return file.trim();
          }).join('\n');
...
git.commit = function (repoPath, amend, message, files) {
  ...
  if (message === undefined) return task.setResult({ error: 'Must specify commit message' });

  if ((!(files instanceof Array) || files.length == 0) && !amend) return task.setResult({ error: 'Must specify files or amend to commit' });

  var updateIndexTask = git.updateIndexFromFileList(repoPath, files).fail(task.setResult).done(function () {
    git('commit ' + (amend ? '--amend' : '') + ' --file=- ', repoPath).always(task.setResult).started(function () {
  ...
  task.start = function () {
    ...
    async.map(files, function (file, callback) {
      fs.exists(path.join(repoPath, file), function (exists) {
        if (exists) toAdd.push(file);else toRemove.push(file);
        callback();
      });
    }, function () {
      ...
      async.parallel([function (done) {
        ...
        git('add ' + toAdd.map(function (file) {
          return '"' + file + '"';
        }).join(' '), repoPath).always(done).start();
      }, function (done) {
        if (toRemove.length == 0) return done();
        git('rm ' + toRemove.map(function (file) {
          return '"' + file + '"';
        }).join(' '), repoPath).always(done).start();
      }], function (err) {
        task.setResult(err);
...
git.getCurrentBranch = function (repoPath) {
  var task = new GitTask();
  var gitTask = git('rev-parse --show-toplevel', repoPath).fail(task.setResult).done(function (rootRepoPath) {

    var HEADFile = path.join(rootRepoPath.trim(), '.git', 'HEAD');
    if (!fs.existsSync(HEADFile)) return task.setResult({ errorCode: 'not-a-repository', error: 'No such file: ' + HEADFile });
