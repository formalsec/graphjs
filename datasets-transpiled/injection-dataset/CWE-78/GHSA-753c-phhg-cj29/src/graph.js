...
module.exports.svg = function (modules, circular, config) {
	...
	return checkGraphvizInstalled(config).then(() => createGraph(modules, circular, config, options));
};
...
module.exports.image = function (modules, circular, imagePath, config) {
	...
	return checkGraphvizInstalled(config).then(() => {
		return createGraph(modules, circular, config, options).then(image => writeFile(imagePath, image)).then(() => path.resolve(imagePath));
	});
};
...
module.exports.dot = function (modules, circular, config) {
	...
	return checkGraphvizInstalled(config).then(() => createGraph(modules, circular, config, options)).then(output => output.toString('utf8'));
};