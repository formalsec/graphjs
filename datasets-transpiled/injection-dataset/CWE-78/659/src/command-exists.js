...
var commandExistsUnix = function (commandName, callback) {
  ...
  fileNotExists(commandName, function (isFile) {
    ...
    if (!isFile) {
      var child = exec('command -v ' + commandName + ' 2>/dev/null' + ' && { echo >&1 \'' + commandName + ' found\'; exit 0; }', function (error, stdout, stderr) {
...
var commandExistsWindows = function (commandName, callback) {
  var child = exec('where ' + commandName, function (error) {
    if (error !== null) {
      callback(null, false);
    } else {
      callback(null, true);
