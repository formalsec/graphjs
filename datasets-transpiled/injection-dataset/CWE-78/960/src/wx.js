...
function reqShortUrl(longurl, cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var requrl = 'https://api.weixin.qq.com/cgi-bin/shorturl' + '?access_token=' + d.access_token;
    request({
      method: 'POST',
      url: requrl,
      form: JSON.stringify({
      ...
      if (e) return cb(e);
      if (d.errcode) {
        return cb(new Error(d.errcode + ': ' + d.errmsg));
      }
      cb(null, d);
    });
  });
...
function reqPostMaterial(type, file, cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var url = 'https://api.weixin.qq.com/cgi-bin/material/add_material' + '?access_token=' + d.access_token;
...
function reqMaterial(mediaid, cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var url = 'https://api.weixin.qq.com/cgi-bin/material/get_material' + '?access_token=' + d.access_token;
    request({
      method: 'POST',
      url: url,
      form: JSON.stringify({
...
function reqMedia(mediaid, cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var url = 'https://api.weixin.qq.com/cgi-bin/media/get' + '?access_token=' + d.access_token + '&media_id=' + mediaid;
    request({
      url: url,
      encoding: null
    }, function (e, r, b) {
...
function reqMaterialList(type, offset, cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var url = 'https://api.weixin.qq.com/cgi-bin/material/batchget_material' + '?access_token=' + d.access_token;
    request({
      method: 'POST',
      url: url,
      form: JSON.stringify({
...
function reqJsApiTicket(cb) {
  wx.getAccessToken(function (e, d) {
    if (e) return cb(e);
    var reqJsApiTicketUrl = 'https://api.weixin.qq.com/cgi-bin/ticket' + '/getticket?access_token=' + d.access_token + '&type=jsapi';
    request({
...
function reqAccessToken(cb) {
  var reqAccessTokenUrl = 'https://api.weixin.qq.com/cgi-bin/token' + '?grant_type=client_credential&appid=' + config.wxm.appid + '&secret=' + config.wxm.appsecret;
  request({
...
function isOf(sub, str) {
  ...
  return i === str.length - sub.length && (str[i - 1] == null || str[i - 1] === '.');
}