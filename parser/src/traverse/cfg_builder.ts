...
function buildCFG(astGraph: Graph): CFGraphReturn {
    ...
    function checkUnknownCalledFunctions(functionName: string, functionContext: number, calledFunctionId: number, parentNodeId: number): void {
        const calls: string[] | undefined = unknownCalls.get(functionName) // calls store the unknown calls to the function received as argument (functionName)
        if (!calls) return;
        calls.forEach((calleeFunction: string) => { // for each unknown call that matches the name
            ...
            const possibleContexts = functionContexts.get(context) ?? [] // the defined function can be defined in the higher contexts of the callee function
            const innerContext = possibleContexts.reverse().find((ctx: number) => ctx === functionContext);
            ...
            graph.addEdge(parentNodeId, calledFunctionId, { type: "CG", label: "CG" })
        ...
        //
            ...
            case "LabeledStatement": {
                ...
                if ((node.type === "FunctionDeclaration" || node.type === "FunctionExpression")) {
                    ...
                    if (node.functionName) checkUnknownCalledFunctions(node.functionName, node.functionContext, node.id, _start.functionNodeId)
