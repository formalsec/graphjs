...
function buildCFG(astGraph: Graph): CFGraphReturn {
    ...
    function checkUnknownCalledFunctions(node: GraphNode, calledFunctionId: number): void {
        const calls: string[] | undefined = node.functionName ? unknownCalls.get(node.functionName) : undefined // calls store the unknown calls to the function received as argument (functionName)
        if (!calls) return;
            ...
            case "LabeledStatement": {
                ...
                if ((node.type === "FunctionDeclaration" || node.type === "FunctionExpression")) {
                    ...
                    if (node.functionName) {
                        checkUnknownCalledFunctions(node, _start.functionNodeId)
                    }
