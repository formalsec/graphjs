...
test("normalize Program", () => {
    const stmtExample = (n: number): ExpressionStatement => ({
    ...
    const childrenExample = (n: number): Normalization => ({ stmts: [stmtExample(n)], expr: null });

    const originalProg: Program = {
        type: "Program",
        sourceType: "script",
...
// function testNormalization(code: string, expectedCode: string) {
